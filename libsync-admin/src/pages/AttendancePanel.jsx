...
export default function AttendancePanel() {
  ...
  const fetchData = async () => {
    try {
      ...
      const qrRes = await axios.get('http://localhost:5000/api/attendance/code');
      setQrCode(qrRes.data.data);
      
      // Fetch today's attendance
      const attendanceRes = await axios.get('http://localhost:5000/api/attendance/today');
      setAttendance(attendanceRes.data.data);
      
      // Fetch attendance stats
      const statsRes = await axios.get('http://localhost:5000/api/attendance/stats');
  ...
  const fetchAttendanceByDate = async (date) => {
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/attendance/range?startDate=${startDate}&endDate=${endDate}`);
